...
4\. Populate your MongoDB database with a list of books. You can use the JSON array in the `books.json` file in the `backend` directory of the project as a starting point.
...
To ensure that data sent to the server is valid, the server uses [Joi](https://joi.dev) for validation. The validation schema for books is defined in the `book.ts` file in the `schemas` directory.
